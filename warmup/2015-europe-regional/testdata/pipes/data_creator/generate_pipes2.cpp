...
ll rrand(ll fMin, ll fMax){
    return (rand() % (fMax - fMin)) + fMin;
}

ll rrand2(ll fMin, ll fMax){
	ll diff = (rand() % (fMax - fMin)) + fMin;
	if (rand() % 2) return -diff;
	return diff;
}
...
int main(){
	...
	int wcood = (W < 100)?1000:10000;
	//int plen = (W < 100)?300:500;
	int plen;
	cin >> plen;
	...
	while (true) {
		...
		// read the wells
		FOR(i,0,W){
			pt x (rrand(-wcood,wcood),rrand(-wcood,wcood));
			wells[i] = x;
		}
		// read the pipes
		FOR (i,0,P) {
			toWell[i] = i%W; //rand() % W;
			pipeEnds[i].x =  wells[toWell[i]].x + rrand2(plen/2,plen);
			pipeEnds[i].y =  wells[toWell[i]].y + rrand2(plen/2,plen);
			if (pipeEnds[i].x < -10000 || pipeEnds[i].x > 10000) i--;
			else if (pipeEnds[i].y < -10000 || pipeEnds[i].y > 10000) i--;
			//cout << wells[toWell[i]].y << " " << pipeEnds[i].y << endl;
		}
		...
		FOR(i,0,P) FOR(j,0,i)
			if (toWell[i] != toWell[j] && isSegmentIntersect(pipeEnds[i],wells[toWell[i]],pipeEnds[j],wells[toWell[j]]) == 1)
				clause(2*i,2*j),clause(notC(2*i),notC(2*j)), cl++;
	
		cerr << cl << endl;
		//if (cl < 50) continue;
		if (cl < min(W,100)) continue;
		kosa(2*P);
		...
		if ((fail && !succ) || (!fail && succ)){
			//return 0;
			FOR(i,0,W) cout << wells[i].x << " " << wells[i].y << endl;
			FOR(i,0,P) cout << toWell[i] << " " << pipeEnds[i].x << " " << pipeEnds[i].y << endl;
			return 0;
		}
	}
