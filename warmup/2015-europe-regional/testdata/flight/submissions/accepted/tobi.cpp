...
double* toSph(double *V1) {
	double *ret = (double*) malloc(sizeof(double) * 2);
	ret[0] = atan2(V1[1], V1[0]);
	ret[1] = atan2(sqrt(V1[0]*V1[0] + V1[1]*V1[1]), V1[2]);
	return ret;
}

double distS(double *P1, double *P2) {
	return acos(cos(P1[1])*cos(P2[1]) + sin(P1[1])*sin(P2[1])*cos(abs(P1[0] - P2[0]))) * 6370;
}
double distC(double *V1, double *V2) {
	return distS(toSph(V1), toSph(V2));
}
