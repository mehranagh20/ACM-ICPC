// Small variation of Florin's correct solution: Only attempt to use
// no singleton or maximum number of singletons, instead of trying
...
int main () {
    ...
    if (!lens.empty())
		for (int j = min((int)lens.size(), P-1); j >= 0; --j)
            if (dp[Z - 1][P - j] != -1) {
