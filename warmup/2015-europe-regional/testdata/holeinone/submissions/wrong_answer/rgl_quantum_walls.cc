...
int verify(vector<pair<vec3,vec3>> walls,vec3 hole,vec3 target,int steps=20){
    if (not --steps) return -1;
    bool anything=false;
    ...
    for (int i=0; i<=walls.size(); i++){
        ...
        for (int j=0; j<walls.size(); j++){
            ...
            if (not (last_distance<dist.first))
                continue; // unreachable edge caused by reflection. ignore.
            if (not anything or (rand() % 100 < 75)) {
        ...
        if (i<walls.size()){
            mat3 ref=reflector(walls[closest.second].first,walls[closest.second].second);
            for (int j=0; j<walls.size(); j++){
...
int main(){
    ...
    for (int x: px)
        for (int y: py)
            for (int z=40; z--;)
                res=max(res,verify(walls,target,vec3{x,y}));
