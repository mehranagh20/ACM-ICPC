...
#include <set>
using namespace std;
long double const infinity=1e60L, impossible=-infinity;
...
long double intersect(vec3 ray,vec3 a,vec3 b){
    ...
    return hypotl(yat/(long double)ray.x,a.x); // looks good; compute exact distance
}

pair<long double,int> verify(vector<pair<vec3,vec3>> walls,vec3 hole,vec3 target){
    long double last_distance=0.L;
    for (int i=0; i<=walls.size(); i++){
        pair<long double,int> closest={hypotl(target.x,target.y),-1};
        for (int j=i; j<walls.size(); j++){
            pair<long double,int> dist={intersect(target,walls[j].first,walls[j].second), j};
...
int main(){
    ...
    for (int x: px) for (int y: py){
        vec3 aim(x,y);
        pair<long double,int> result=verify(walls,target,aim);
